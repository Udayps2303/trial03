"""Script to predict all morphological features using a trained BiLSTM model."""
...
def predictMorphFeaturesForSample(lcats, genders, numbers, persons, cases, vibhs, sampleLen, index2Lcat, index2Gender, index2Number, index2Person, index2Case, index2Vibh):
    predictedTags = ''
    for index in range(sampleLen):
        predictedTags += index2Lcat[np.argmax(lcats[index])] + '\t' + index2Gender[np.argmax(genders[index])] + '\t' + index2Number[np.argmax(
            numbers[index])] + '\t' + index2Person[np.argmax(persons[index])] + '\t' + index2Case[np.argmax(cases[index])] + '\t' + index2Vibh[np.argmax(vibhs[index])] + '\n'
    return predictedTags


def predictAllMorphFeatures(trainedModel, sentenceVectors, charSeqVectors, index2Lcat, index2Gender, index2Number, index2Person, index2Case, index2Vibh, lines, maxSentenceLength):
    lcats, genders, numbers, persons, cases, vibhs = trainedModel.predict(
        [charSeqVectors, sentenceVectors])
    sentenceLengths, greaterDict = sentenceLengthsInLines(
        lines, maxSentenceLength)
    predictedTags = ''
    assert lcats.shape[0] == genders.shape[0] == numbers.shape[0] == persons.shape[0] == cases.shape[0] == vibhs.shape[0]
    for indexSample in range(lcats.shape[0]):
        predictedTags += predictMorphFeaturesForSample(
            lcats[indexSample], genders[indexSample], numbers[indexSample], persons[indexSample], cases[indexSample], vibhs[indexSample], sentenceLengths[indexSample], index2Lcat, index2Gender, index2Number, index2Person, index2Case, index2Vibh)
        predictedTags += '\n'
...
def main():
    ...
    predictedTags, greaterDict = predictAllMorphFeatures(trainedModel, finalSentenceVectors,
                                                         finalCharSequences, index2Lcat, index2Gender, index2Number, index2Person, index2Case, index2Vibh, lines, maxSentenceLength)
