...
morph_feats = ['case', 'gender', 'number', 'person', 'lcat', 'vibhakti']
...
def find_gold_pred_paths_and_create_reports(gold_paths, pred_paths, report_folder):
    ...
    for gold_path in gold_paths:
        ...
        morph_feat = search('|'.join(morph_feats), gold_file_name).group(0)
        for pred_path in pred_paths:
            pred_file_name = pred_path[pred_path.rfind('/') + 1:]
            print(gold_file_name, pred_file_name)
            if domain in pred_file_name and morph_feat in pred_file_name:
                print(gold_file_name, pred_file_name, domain, morph_feat)
                report_path = os.path.join(report_folder, 'report-' + domain + '-' + morph_feat + '.txt')
                os.system('python precision_recall_score_pos.py ' + gold_path + ' ' + pred_path + ' ' + report_path)
                break
