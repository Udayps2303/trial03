...
def findPredictedTagsForSample(posTags, chunkTags, sampleLen, indexToPOS, indexToChunk):
    predictedTags = ''
    for index in range(sampleLen):
        predictedTags += indexToPOS[np.argmax(posTags[index])] + \
            '\t' + \
            indexToChunk[np.argmax(chunkTags[index])] + '\n'
    return predictedTags


def predictPOSChunkTags(trainedModel, sentenceVectors, charSeqVectors, indexToPOS, indexToChunk, lines, maxSentenceLength):
    posTags, chunkTags = trainedModel.predict(
    ...
    for indexSample in range(posTags.shape[0]):
        predictedTags += findPredictedTagsForSample(
            posTags[indexSample], chunkTags[indexSample], sentenceLengths[indexSample], indexToPOS, indexToChunk)
        predictedTags += '\n'
...
def main():
    ...
    #indexToPOS = loadObjectToPickleFile('pos-tags-to-index-tb-jud.pkl')
    indexToPOS = loadObjectToPickleFile('pos-tags-to-index-tb-ilmt.pkl')
    #indexToChunk = loadObjectToPickleFile('chunk-tags-to-index-tb-jud.pkl')
    ...
    del word2vecModel
    #exit(1)
    #print(finalCharSequences[0])
    #print(finalSentenceVectors[0])
    #exit(1)
    ...
    predictedTags, greaterDict = predictPOSChunkTags(trainedModel, finalSentenceVectors,
                                                     finalCharSequences, indexToPOS, indexToChunk, lines, maxSentenceLength)
