...
def createVectorsForDataAndLabels(inputFilePath, word2Index, wordEmbeddings, char2Index, pos2Index):
    allsentenceVectors = list()
    sentenceVectors = list()
    allPOSTagVectors, posTagsForSent = list(), list()
    ...
    with open(inputFilePath) as test:
        for line in test:
            if line.strip():
                ...
                sentenceVectors, posTagsForSent = list(), list()
    ...
    return finalCharSequences, finalSentenceVectors, finalPOSTagSequences
...
def trainModelUsingBiLSTM(maxWordLen, maxSentLen, sentenceVectors, charSequences, posTags, totalChars, totalPOS, weightFile, valData):
...
#    charMask = Masking(mask_value=0)(charSeq)
    ...
    finalModel = Model(
        inputs=[charSeq, wordSeq], outputs=activationForPOS)
    ...
    batch_size = 16
    valData[2] = valData.reshape((-1, maxSentLen, 1))
    checkpointCallback = ModelCheckpoint(weightFile, monitor='val_loss', verbose=0,
                                         save_best_only=True, save_weights_only=False, mode='auto', save_freq='epoch')
    finalModel.fit([charSequences, sentenceVectors], posTags,
                   batch_size=batch_size, epochs=30, callbacks=[checkpointCallback], validation_data=([valData[0], valData[1]], valData[2]))
    return finalModel
...
def main():
    ...
    if not args.inp or not args.val or not args.mem or not args.w2i or not args.c2i or not args.wt:
        ...
        finalCharSequences, finalSentenceVectors, finalPOSTagSequences = createVectorsForDataAndLabels(
            args.inp, word2Index, memoryMapped, char2Index, pos2Index)
        # del word2vecModel
        validationData = createVectorsForDataAndLabels(
                            args.val, word2Index, memoryMapped, char2Index, pos2Index)
        del memoryMapped
        ...
        biLSTM = trainModelUsingBiLSTM(maxWordLength, maxSentenceLength, finalSentenceVectors, finalCharSequences,
                                       finalPOSTagSequences, len(char2Index), len(pos2Index), args.wt, validationData)
